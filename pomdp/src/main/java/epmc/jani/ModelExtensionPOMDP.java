...
public final class ModelExtensionPOMDP implements ModelExtensionSemantics {
    ...
    public void parseAfter() {
        if (node instanceof Edge) {
            Edge edge = (Edge) node;
            ensure(edge.getRate() == null, ProblemsJANIPOMDP.JANI_MDP_EDGE_FORBIDS_RATE);
        }
        if (node instanceof Location) {
            Location location = (Location) node;
            ensure(location.getTimeProgress() == null, ProblemsJANIPOMDP.JANI_MDP_DISALLOWED_TIME_PROGRESSES);
        }
    }
